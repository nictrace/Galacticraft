...
//	            return Vec3.createVectorHelper(f4 * var21, f5 * var21, f6 * var21);
    ...
    public static List<Integer> getPossibleDimensionsForSpaceshipTier(int tier, EntityPlayerMP playerBase)
    {
        ...
        if(playerBase == null){
        	GCLog.info("What?! playerBase == null");
        	return temp;
        }
        if(playerBase.worldObj.provider != null){
            ...
            if(departure_cb == null){
            	if (playerBase.worldObj.provider instanceof IOrbitDimension){
                    ...
                    departure_cb = getReachableCelestialBodiesForDimensionID(dataCurrent.getHomePlanet());
            	}
            }
        }
        else GCLog.info("WARN: playerBase.worldObj.provider == null");
...
private static int getRelativeTier(CelestialBody departure_cb, CelestialBody target_cb) {
		...
		if(departure_cb == null) return target_cb.getTierRequirement();
