...
public class CloudRendererVenus extends IRenderHandler
{
    ...
    public void render(float partialTicks, WorldClient world, Minecraft mc)
    {
        ...
        float f1 = (float)(this.mc.renderViewEntity.lastTickPosY + (this.mc.renderViewEntity.posY - this.mc.renderViewEntity.lastTickPosY) * (double)partialTicks);
        ...
        for (int count = 0; count < 4; ++count)
        {
            ...
            for (int k = 0; k < 2; ++k)
            {
                ...
                double d0 = (double)(cloudTickCounter * (1.0F - count / 5.0F) + (count / 5.0F) * partialTicks + count * 250000);
                double d1 = (this.mc.renderViewEntity.prevPosX + (this.mc.renderViewEntity.posX - this.mc.renderViewEntity.prevPosX) * (double)partialTicks + d0 * 0.029999999329447746D) / (double)f2;
                double d2 = (this.mc.renderViewEntity.prevPosZ + (this.mc.renderViewEntity.posZ - this.mc.renderViewEntity.prevPosZ) * (double)partialTicks) / (double)f2 + 0.33000001311302185D;
